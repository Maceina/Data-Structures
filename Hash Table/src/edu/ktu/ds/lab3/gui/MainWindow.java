...
import edu.ktu.ds.lab3.maceina.SimpleBenchmark;
...
import edu.ktu.ds.lab3.maceina.Computer;
import edu.ktu.ds.lab3.maceina.ComputerGenerator;
...
public class MainWindow extends BorderPane implements EventHandler<ActionEvent> {
    ...
    private static final Computer TEST_COMPUTER_1 = new Computer("Apple", "Mac", 1999, 400);
    private static final Computer TEST_COMPUTER_2 = new Computer("Dell", "x522", 1999, 420);
    private static final Computer TEST_COMPUTER_3 = new Computer("Asus", "p15", 1999, 355);
    private static final String[] TEST_COMPUTERS_IDS = {"TA101", "TA199", "TA102"};
    ...
    private ParsableMap<String, Computer> map;
    private int sizeOfInitialSubSet, sizeOfGenSet, colWidth, initialCapacity;
    private float loadFactor;
    private HashType ht = HashType.DIVISION;
    private final ComputerGenerator computersGenerator = new ComputerGenerator();
    ...
    private void dataPut() {
        map.put(TEST_COMPUTERS_IDS[0], TEST_COMPUTER_1);
        map.put(TEST_COMPUTERS_IDS[1], TEST_COMPUTER_2);
        map.put(TEST_COMPUTERS_IDS[2], TEST_COMPUTER_3);
    ...
    private void containsValue() {
        ...
        Computer testComputer = new Computer(inputData);

        ParsableHashMap hashMap = (ParsableHashMap) map;
        if (hashMap.containsValue(testComputer)) {
    ...
    private void putIfAbsent() {
        ...
        Computer inputComputerData = new Computer(inputDataArray[1]);

        switch (cmbCollisionTypes.getSelectionModel().getSelectedIndex()) {
            case 0:
                ParsableHashMap hashMap = (ParsableHashMap) map;
                testObject = hashMap.putIfAbsent(inputKey, inputComputerData);
                break;

            case 1:
                ParsableHashMapOa hashMap2 = (ParsableHashMapOa) map;
                testObject = hashMap2.putIfAbsent(inputKey, inputComputerData);
                break;
        }
        if (testObject == null) {
            KsGui.oun(taEvents, inputComputerData, MESSAGES.getString("putIfAbsentTrue"));
        } else {
    ...
    private void mapGeneration(String filePath) {
        ...
        if (filePath == null) {
            Computer[] computersArray = computersGenerator.generateShuffleComputersAndIds(sizeOfGenSet, sizeOfInitialSubSet);
            for (Computer c : computersArray) {
                map.put(
                        computersGenerator.getComputerId(), //raktas
                        c);
            }
    ...
    private void mapPut() {
        Computer computer = computersGenerator.getComputer();
        map.put(
                computersGenerator.getComputerId(), // Raktas
                computer);
        table.formTable(map.getMaxChainSize() * 2 + 1, colWidth);
        ...
        table.setItems(FXCollections.observableArrayList(modelList));
        updateHashtableParameters(true);
        KsGui.oun(taEvents, computer, MESSAGES.getString("mapPut"));
    }
    ...
    private void createMap() {
        switch (cmbCollisionTypes.getSelectionModel().getSelectedIndex()) {
            case 0:
                map = new ParsableHashMap<>(String::new, Computer::new, initialCapacity, loadFactor, ht);
                break;
            case 1:
                map = new ParsableHashMapOa<>(String::new, Computer::new, initialCapacity, loadFactor, ht);
                break;
            // ...
