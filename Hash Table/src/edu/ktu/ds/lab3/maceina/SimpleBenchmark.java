...
public class SimpleBenchmark {
    ...
    private final String[] BENCHMARK_NAMES = {"Put", "ComputerRemKTU", " ComputerRemJava",
        "DictionaryPutKTU", "DictionaryContainsJava"};
    private final int[] COUNTS = {10000, 20000, 40000, 80000};

    private final ParsableHashMap<String, Computer> hashMapPut
            = new ParsableHashMap<>(String::new, Computer::new, 10, 0.75f, HashType.DIVISION);

    private final edu.ktu.ds.lab3.utils.HashMap<String, Computer> hashMapComputerRemoveKtu
            = new edu.ktu.ds.lab3.utils.HashMap<>(10, 0.75f, HashType.DIVISION);

    private final HashMap<String, Computer> hashMapComputerRemoveJava = new HashMap<>();
    ...
    public void benchmark() throws InterruptedException, FileNotFoundException {
        try {
            ...
            for (int k : COUNTS) {
                ...
                Computer[] computerArray = ComputerGenerator.generateShuffleComputers(k);
                String[] computerArray2 = ComputerGenerator.generateShuffleIds(k);

                Computer[] computersSearchArray = computerArray;
                String[] computersSearchIdsArray = computerArray2;

                shuffleProc(computersSearchArray);
                shuffleProc(computersSearchIdsArray);
                ...
                hashMapComputerRemoveKtu.clear();
                hashMapComputerRemoveJava.clear();
                timekeeper.startAfterPause();
                timekeeper.start();

                for (int i = 0; i < k; i++) {
                    hashMapComputerRemoveKtu.put(computerArray2[i], computerArray[i]);
                    hashMapComputerRemoveJava.put(computerArray2[i], computerArray[i]);
                }
                timekeeper.finish(BENCHMARK_NAMES[0]);

                for (int i = 0; i < k; i++) {
                    hashMapComputerRemoveKtu.remove(computersSearchIdsArray[i]);
                }
                timekeeper.finish(BENCHMARK_NAMES[1]);

                for (int i = 0; i < k; i++) {
                    hashMapComputerRemoveJava.remove(computersSearchIdsArray[i]);
                }
                ...
                for (int i = 0; i < k; i++) {
                    hashMapComputerRemoveJava.remove(wordStringArray[i]);
                }
