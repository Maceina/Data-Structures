...
import edu.ktu.ds.lab2.Maceina.Computer;
import edu.ktu.ds.lab2.Maceina.ComputerGenerator;
import edu.ktu.ds.lab2.Maceina.SimpleBenchmark;
 ...
 */
public class MA_MainWindow extends BorderPane implements EventHandler<ActionEvent> {

    private static final ResourceBundle MESSAGES = ResourceBundle.getBundle("edu.ktu.ds.lab2.gui.MA_messages");
    ...
    private BstSet<Computer> testSet;
    private static ParsableSortedSet<Computer> computerSet;
    private ComputerGenerator computerGenerator = new ComputerGenerator();

    private int sizeOfInitialSubSet, sizeOfGenSet, sizeOfLeftSubSet;
    private double shuffleCoef;
    private final String[] errors;

    public MA_MainWindow(Stage stage) {
        this.stage = stage;
        errors = new String[]{
    ...
    private void handleButtons(Object source) throws ValidationException {
        if (source.equals(paneButtons.getButtons().get(0))) {
            ...
            pollLast();
        } else if (source.equals(paneButtons.getButtons().get(14))) {
            treeHeight();
        } else if (source.equals(paneButtons.getButtons().get(15))) {
            iterRemove();
        }
//        else if (source.equals(paneButtons.getButtons().get(12))
...
//        }
    ...
    private void treeGeneration(String filePath) throws ValidationException {
        ...
        if (filePath == null) {
            computerArray = computerGenerator.generateShuffle(sizeOfGenSet, sizeOfInitialSubSet, shuffleCoef);
            paneParam1.getTfOfTable().get(2).setText(String.valueOf(sizeOfLeftSubSet));
        } else { // Skaitoma is failo
            computerSet.load(filePath);
            computerArray = new Computer[computerSet.size()];
            int i = 0;
            for (Object o : computerSet.toArray()) {
                computerArray[i++] = (Computer) o;
            }
            // Skaitant iš failo išmaišoma standartiniu Collections.shuffle metodu.
            Collections.shuffle(Arrays.asList(computerArray), new Random());
        }

        // Išmaišyto masyvo elementai surašomi i aibę
        computerSet.clear();
        Arrays.stream(computerArray).forEach(computerSet::add);

        // Išvedami rezultatai
        // Nustatoma, kad eilutės pradžioje neskaičiuotų išvedamų eilučių skaičiaus
        KsGui.setFormatStartOfLine(true);
        KsGui.oun(taOutput, computerSet.toVisualizedString(tfDelimiter.getText()),
    ...
    private void treeAdd() throws ValidationException {
        KsGui.setFormatStartOfLine(true);
        Computer computer = computerGenerator.takeCar();
        computerSet.add(computer);
        paneParam1.getTfOfTable().get(2).setText(String.valueOf(--sizeOfLeftSubSet));
        KsGui.oun(taOutput, computer, MESSAGES.getString("setAdd"));
        KsGui.oun(taOutput, computerSet.toVisualizedString(tfDelimiter.getText()));
        KsGui.setFormatStartOfLine(false);
    }
    ...
    private void treeRemove() {
        ...
        if (computerSet.isEmpty()) {
            ...
            int nr = new Random().nextInt(computerSet.size());
            Computer computer = (Computer) computerSet.toArray()[nr];
            computerSet.remove(computer);
            KsGui.oun(taOutput, computer, MESSAGES.getString("setRemoval"));
            KsGui.oun(taOutput, computerSet.toVisualizedString(tfDelimiter.getText()));
        }
        KsGui.setFormatStartOfLine(false);
    }
    ...
    private void treeIteration() {
        ...
        if (computerSet.isEmpty()) {
            ...
            KsGui.oun(taOutput, computerSet, MESSAGES.getString("setIterator"));
        }
        KsGui.setFormatStartOfLine(false);
    }
    ...
    private void defSet(int k) {
        KsGui.setFormatStartOfLine(true);
        edu.ktu.ds.lab2.utils.Set<Computer> set;
        if (computerSet.isEmpty()) {
            KsGui.ounerr(taOutput, MESSAGES.getString("setIsEmpty"));
        } else {
            if (k == 0) {
                set = computerSet.headSet(test);
            } else if (k == 1) {
                set = computerSet.tailSet(test);
            } else {
                set = computerSet.subSet(test, test1);
            }
    ...
    private void customSet() {
        ...
        edu.ktu.ds.lab2.utils.SortedSet<Computer> set;
    ...
    private void higher() {
        ...
        if (computerSet.isEmpty()) {
            ...
            if (set == null) {
                KsGui.oun(taOutput, MESSAGES.getString("testHigherNull"));
            } else {
...
//            boolean addAllB = testing.addAll(beginer);
            boolean addAllB = testing.addAll(testSet);
            if (addAllB) {
                KsGui.oun(taOutput, MESSAGES.getString("AddAllTrue"));
            } else {
                KsGui.oun(taOutput, MESSAGES.getString("AddAllFalse"));
            }
    ...
    private void containsAll() {
        ...
        boolean contAll = testing.containsAll(testSet);
    ...
    private void treeHeight() {
        ...
        int height = testing.findHeight();

        KsGui.oun(taOutput, height, MESSAGES.getString("height"));
    }

    private void iterRemove() {
        ParsableBstSet<Computer> testing;
        testing = (ParsableBstSet<Computer>) computerSet;
        int k = 0;
        Iterator<Computer> it = testing.iterator();
        while (it.hasNext()) {
            ...
            if (comp.compareTo(test1) == 0) {
                k = 1;
            }

        }
        KsGui.oun(taOutput, MESSAGES.getString("iterRem"));
        KsGui.oun(taOutput, computerSet.toVisualizedString(tfDelimiter.getText()));

    }

//-------------------------------------------------------------------------------------------------------------------------   
//            ParsableBstSet<Proccesor> testing;
//            testing = (ParsableBstSet<Proccesor>) proccesorSet;  
//-------------------------------------------------------------------------------------------------------------------------  
    ...
    private void createTree() throws ValidationException {
        switch (cmbTreeType.getSelectionModel().getSelectedIndex()) {
            case 0:
                computerSet = new ParsableBstSet<>(Computer::new);
                break;
            case 1:
                computerSet = new ParsableAvlSet<>(Computer::new);
                break;
            default:
                disableButtons(true);
    ...
    private void TestChooseMenu(boolean type) throws ValidationException, FileNotFoundException {
        ...
        if (file != null) {
            ...
            test = new Computer(line, type);
            line = input.nextLine();
            test1 = new Computer(line, type);

            KsGui.oun(taOutput, test, MESSAGES.getString("testAdd1"));
            KsGui.oun(taOutput, test1, MESSAGES.getString("testAdd2"));
        }
    }
    ...
    private void TestSetChooseMenu(boolean type) throws ValidationException, FileNotFoundException {
        ...
        if (file != null) {
            ...
            while (input.hasNext()) {
                String line = input.nextLine();
                testSet.add(new Computer(line, type));
            }
...
//            Files.write(Paths.get(filePath), (Iterable)proccesorSet);
    ...
    public static void createAndShowGui(Stage stage) {
        ...
        MA_MainWindow window = new MA_MainWindow(stage);
